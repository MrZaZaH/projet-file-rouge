---
name: mise-a-jour-v2
description: Mettre à jour la documentation technique (docs/archives/mecanismes-v2/) après des changements dans le code ou la BDD. Déclenché par "mise à jour v2" suivi d'une description des changements.
license: MIT
...
J'identifie les fichiers `docs/archives/mecanismes-v2/*.md` impactés par des changements décrits par l'utilisateur, je repère les sections obsolètes (anciens noms, comportements, chemins, extraits de code), et j'applique les corrections sans modifier la structure du fichier.
...
1. **Identifier les fichiers impactés** : en fonction de la description des changements, déterminer quels fichiers `docs/archives/mecanismes-v2/*.md` contiennent des informations devenues obsolètes.
